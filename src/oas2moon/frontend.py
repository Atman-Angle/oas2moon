"""Versioned project-owned Frontend Model.

The model is produced by the formal MoonBit frontend adapter. Every value here
is already normalized: local ``$ref`` targets are recorded as identities,
OpenAPI-specific syntax is represented explicitly, and nothing in this module
imports or exposes mooncontract types.
"""
...
def _as_int(value: Any, where: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise FrontendModelError(f"{where}: expected an integer, got {type(value).__name__}")
    return value
...
class Finding:
    ...
    def from_json(value: Any, where: str) -> "Finding":
        ...
        return Finding(
            ...
            operation_id=_as_optional_string(node.get("operation_id"), f"{where}.operation_id"),
        )
...
class FrontendModel:
    ...
    @property
    def frontend_model_version(self) -> int:
...
def parse(document: Any) -> FrontendModel:
    ...
    return FrontendModel(
        model_version=_as_int(
            root.get("frontendModelVersion"), "#.frontendModelVersion"
        ),
        ...
        security_schemes=tuple(
            SecurityScheme.from_json(name, schemes[name], f"#.securitySchemes.{name}")
        ...
        schemas=tuple(
            (name, Schema.from_json(schemas[name], f"#.schemas.{name}"))
