...
- `role` — `real-world` for the API subsets the published headline numbers come
  from, `control` for local fixtures that only guard the machinery. Controls are
  reported in their own block and never pad the real-world totals;
...
1. Decide the subset. "GitHub REST" is far outside V1, so a subset (a tag, a
   path prefix, a curated operation list) is what actually gets measured — state
   which one it is in `subset`.
