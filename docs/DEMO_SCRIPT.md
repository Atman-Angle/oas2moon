...
Open [cross-platform-ci run 35177945624](https://github.com/Atman-Angle/oas2moon/actions/runs/35177945624). Its API-accessible result is `success` for `Verify (Ubuntu)` and `Verify (Windows)` on commit `040f488`. The workflow runs the same test and demo gates. If the link is unavailable during a presentation, say so and show the workflow file; do not substitute a guessed result.
